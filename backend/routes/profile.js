  ...
  try {
    const [rows] = await query(
      `SELECT u.id, u.name, u.email, u.company, u.contact_phone, u.plan_id, 
              ...
              u.rcs_limit, u.wa_limit, u.sms_limit, u.voice_limit, u.is_proero_enabled, u.is_smm_enabled, u.api_key, u.dlr_webhook_url,
...
router.put('/', authenticate, async (req, res) => {
  ...
  if (req.body.dlr_webhook_url !== undefined) {
    updates.push('dlr_webhook_url = ?');
    values.push(req.body.dlr_webhook_url ? req.body.dlr_webhook_url.trim() : null);
  }
  ...
  try {
    ...
    const [updated] = await query(
      `SELECT id, name, email, company, contact_phone, plan_id, credits_available, wallet_balance, channels_enabled, sms_transactional_price, sms_promotional_price, sms_service_price, pe_id, hash_id, is_proero_enabled, is_smm_enabled, dlr_webhook_url, api_key
       FROM users WHERE id = ?`,
      [req.user.id]
    );

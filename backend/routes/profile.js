...
router.put('/', authenticate, async (req, res) => {
  ...
  try {
    ...
    const [updated] = await query(
      `SELECT id, name, email, company, contact_phone, plan_id, credits_available, wallet_balance, channels_enabled, sms_transactional_price, sms_promotional_price, sms_service_price, pe_id, hash_id, is_proero_enabled, is_smm_enabled, dlr_webhook_url, api_key, is_dinstar_enabled
       FROM users WHERE id = ?`,
      [req.user.id]
    );

...
// Stale-While-Revalidate Cache for dashboard statistics
const statsCacheMap = new Map();
const STATS_CACHE_TTL = 300000; // Cache data is kept up to 5 minutes
const STATS_REFRESH_THRESHOLD = 60000; // Trigger background update after 1 minute

// Helper to fetch super-admin/reseller stats
async function fetchSuperAdminStatsData(isReseller, resellerId) {
    // 1. Basic Counters
    ...
    let msgSql = `
        SELECT 
            ...
            COALESCE(SUM(CASE WHEN created_at >= CURDATE() THEN GREATEST(COALESCE(audience_count, 0), COALESCE(recipient_count, 0)) ELSE 0 END), 0) as today,
    ...
    let finSql = `
        SELECT 
            ...
            COALESCE(SUM(CASE WHEN created_at >= CURDATE() THEN amount ELSE 0 END), 0) as revenue_today,
            COALESCE(SUM(CASE WHEN created_at >= DATE_FORMAT(NOW(), '%Y-%m-01') THEN amount ELSE 0 END), 0) as revenue_month
        FROM transactions 
        WHERE type = 'credit'
    `;
    if (isReseller) {
    ...
    let creditSql = `
        SELECT 
            ...
            COALESCE(SUM(CASE WHEN created_at >= CURDATE() THEN amount ELSE 0 END), 0) as consumed_today,
            COALESCE(SUM(CASE WHEN created_at >= DATE_FORMAT(NOW(), '%Y-%m-01') THEN amount ELSE 0 END), 0) as consumed_month
        FROM transactions 
        WHERE type = 'debit'
    `;
    if (isReseller) {
    ...
    const responseData = {
        ...
        stats: {
            ...
            totalConversations: Number(msgStats[0]?.total || 0),
            ...
            weeklyChats: weeklyMessages.map(m => ({ day: m.day, count: m.messages })),
            weeklyMessages,
            channelUsage,
...
// Helper to fetch client-specific stats
async function fetchClientStatsData(userId) {
    ...
    // 3. Channel Distribution & Analytics
...
router.get('/super-admin', authenticate, async (req, res) => {
    try {
        ...
        const cacheKey = `super-admin:${req.user.role}:${resellerId || 'all'}`;

        const cached = statsCacheMap.get(cacheKey);
        const now = Date.now();

        if (cached && (now - cached.timestamp < STATS_CACHE_TTL)) {
            // Trigger background async refresh if refresh threshold is exceeded and not currently updating
            if (now - cached.timestamp > STATS_REFRESH_THRESHOLD && !cached.isUpdating) {
                cached.isUpdating = true;
                fetchSuperAdminStatsData(isReseller, resellerId)
                    .then(freshData => {
                        statsCacheMap.set(cacheKey, { data: freshData, timestamp: Date.now(), isUpdating: false });
                    })
                    .catch(err => {
                        console.error('Background Super-Admin Stats Update Error:', err);
        ...
        // Fetch synchronously if there is no cache
        const responseData = await fetchSuperAdminStatsData(isReseller, resellerId);
        statsCacheMap.set(cacheKey, { data: responseData, timestamp: now, isUpdating: false });
...
router.get('/stats', authenticate, async (req, res) => {
    try {
        ...
        if (cached && (now - cached.timestamp < STATS_CACHE_TTL)) {
            ...
            if (now - cached.timestamp > STATS_REFRESH_THRESHOLD && !cached.isUpdating) {
                cached.isUpdating = true;
                fetchClientStatsData(userId)
                    .then(freshData => {
                        statsCacheMap.set(cacheKey, { data: freshData, timestamp: Date.now(), isUpdating: false });
                    })
                    .catch(err => {
                        console.error('Background Client Stats Update Error:', err);
                        cached.isUpdating = false;
                    });
            }
            return res.json(cached.data);
        ...
        // Fetch synchronously if there is no cache
        const responseData = await fetchClientStatsData(userId);
        statsCacheMap.set(cacheKey, { data: responseData, timestamp: now, isUpdating: false });
        res.json(responseData);
    } catch (err) {
...
router.get('/usage-ledger', authenticate, async (req, res) => {
    try {
        ...
        const startDate = `${yearVal}-${String(monthVal).padStart(2, '0')}-01 00:00:00`;
        const endDate = monthVal === 12 
            ? `${yearVal + 1}-01-01 00:00:00` 
            : `${yearVal}-${String(monthVal + 1).padStart(2, '0')}-01 00:00:00`;
        ...
        // console.log('[UsageLedger] Count SQL:', countSql, 'Params:', params);
        ...
        const sql = `
            SELECT 
                ...
                COALESCE((SELECT SUM(GREATEST(COALESCE(audience_count, 0), COALESCE(recipient_count, 0))) FROM campaigns WHERE user_id = u.id AND channel = 'sms' AND created_at >= ? AND created_at < ?), 0) as sms_bulk,
                COALESCE((SELECT SUM(GREATEST(COALESCE(audience_count, 0), COALESCE(recipient_count, 0))) FROM api_campaigns WHERE user_id = u.id AND channel = 'sms' AND created_at >= ? AND created_at < ?), 0) as sms_api,
                COALESCE((SELECT SUM(GREATEST(COALESCE(audience_count, 0), COALESCE(recipient_count, 0))) FROM campaigns WHERE user_id = u.id AND channel = 'whatsapp' AND created_at >= ? AND created_at < ?), 0) as wa_bulk,
                COALESCE((SELECT SUM(GREATEST(COALESCE(audience_count, 0), COALESCE(recipient_count, 0))) FROM api_campaigns WHERE user_id = u.id AND channel = 'whatsapp' AND created_at >= ? AND created_at < ?), 0) as wa_api,
                COALESCE((SELECT SUM(GREATEST(COALESCE(audience_count, 0), COALESCE(recipient_count, 0))) FROM campaigns WHERE user_id = u.id AND channel = 'rcs' AND created_at >= ? AND created_at < ?), 0) as rcs_bulk,
                COALESCE((SELECT SUM(GREATEST(COALESCE(audience_count, 0), COALESCE(recipient_count, 0))) FROM api_campaigns WHERE user_id = u.id AND channel = 'rcs' AND created_at >= ? AND created_at < ?), 0) as rcs_api,

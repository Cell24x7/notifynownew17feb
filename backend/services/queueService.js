...
const processBatch = async ({ campaignTable, queueTable, logsTable, name: processorName }) => {
    ...
    try {
        ...
        const sql = `
             ...
             wc.provider as wa_provider, wc.api_key as wa_api_key, wc.wa_token, wc.ph_no_id as wa_ph_no_id, wc.wa_biz_accnt_id as wa_biz_accnt_id, wc.customer_id as customer_id,
        ...
        while (true) {
            ...
            if (!candidates || candidates.length === 0) {
                // No pending items found across any running campaign — all done
                break; 
            }
            ...
            // Do NOT break based on total count < DRIP_BATCH_SIZE — some campaigns
            // may have fewer rows than their slice but others may still have more.
            const anyHadRows = runningCampsList.some(camp => {
                return candidates.some(c => c.campaign_id === camp.id);
            });
            if (!anyHadRows) break;

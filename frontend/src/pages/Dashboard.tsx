...
export default function Dashboard() {
  ...
  const pieData = Object.entries(stats.channelDistribution).map(([channel, value]) => ({
  ...
  // For now, we simulate these breakdown stats from the total if not provided by backend explicitly
  const totalMessages = stats.totalConversations * 12; 
  const totalDelivered = Math.floor(totalMessages * 0.95);
  const totalBotHandled = Math.floor(stats.totalConversations * 0.4);
  const totalHumanHandled = stats.totalConversations - totalBotHandled;

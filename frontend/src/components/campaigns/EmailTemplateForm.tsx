    ...
    <div className="space-y-5 sm:space-y-8 animate-in fade-in duration-500">
      ...
      <div className="space-y-2 sm:space-y-3">
        <Label className="text-xs sm:text-sm font-bold text-slate-700 flex items-center gap-2">
          <Layout className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary" /> Choose a Quick Design
        </Label>
        <div className="grid grid-cols-3 gap-2 sm:gap-4">
          {Object.entries(EMAIL_TEMPLATES).map(([key, template]) => (
            <div 
              ...
              className="group cursor-pointer p-2.5 sm:p-4 rounded-xl sm:rounded-2xl border-2 border-slate-100 bg-white hover:border-primary/30 hover:bg-primary/5 transition-all flex flex-col items-center gap-1.5 sm:gap-2 text-center"
            >
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-100 group-hover:bg-primary/10 flex items-center justify-center text-slate-500 group-hover:text-primary transition-colors">
                 <Mail className="w-4 h-4 sm:w-5 sm:h-5" />
              </div>
              <span className="text-[10px] sm:text-xs font-bold capitalize">{key}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4 sm:space-y-6">
        <div className="space-y-2 sm:space-y-3">
            <Label className="text-xs sm:text-sm font-bold text-slate-700">Template Name</Label>
            <Input 
                placeholder="e.g. welcome_email_v1"
                value={data.name || ''}
                onChange={(e) => handleChange('name', e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '_'))}
                className="rounded-lg sm:rounded-xl h-10 sm:h-12 bg-white shadow-sm border-slate-100 text-sm"
            />
        </div>

        <div className="space-y-2 sm:space-y-3">
            <Label className="text-xs sm:text-sm font-bold text-slate-700">Email Subject Line</Label>
            <Input 
        ...
        <div className="space-y-2 sm:space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <Label className="text-xs sm:text-sm font-bold text-slate-700">Email Body Content</Label>
            ...
            <Tabs value={activeTab} onValueChange={(v: any) => setActiveTab(v)} className="w-full">
                <TabsList className="bg-slate-100 p-1 rounded-lg sm:rounded-xl mb-2 sm:mb-3 w-full sm:w-auto">
                    <TabsTrigger value="visual" className="rounded-md sm:rounded-lg px-3 sm:px-4 font-bold text-[10px] sm:text-[11px] flex-1 sm:flex-none"><Sparkles className="w-3 h-3 mr-1 sm:mr-1.5" /> Visual Editor</TabsTrigger>
                    <TabsTrigger value="code" className="rounded-md sm:rounded-lg px-3 sm:px-4 font-bold text-[10px] sm:text-[11px] flex-1 sm:flex-none"><Code className="w-3 h-3 mr-1 sm:mr-1.5" /> HTML Code</TabsTrigger>
                </TabsList>

                <TabsContent value="visual" className="mt-0">
                    <div className="rounded-xl sm:rounded-2xl border border-slate-100 overflow-hidden bg-white shadow-sm h-[250px] sm:h-[350px] md:h-[400px]">
                        {/* Use simplified toolbar on mobile via CSS media query */}
                        <ReactQuill 
                            theme="snow" 
                            value={data.body || ''} 
                            onChange={(content) => handleChange('body', content)}
                            modules={typeof window !== 'undefined' && window.innerWidth < 640 ? quillModulesMobile : quillModules}
                            className="h-[200px] sm:h-[300px] md:h-[350px] visual-email-editor"
                        />
                    </div>
                </TabsContent>
                ...
                <TabsContent value="code" className="mt-0">
                    <textarea 
                        className="w-full h-[250px] sm:h-[350px] md:h-[400px] p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 bg-slate-900 text-emerald-400 font-mono text-xs sm:text-sm focus:ring-primary focus:border-primary outline-none resize-none"
                        value={data.body || ''}
      ...
      <div className="p-4 sm:p-6 rounded-xl sm:rounded-2xl bg-indigo-50/30 border border-indigo-100 shadow-sm space-y-3 sm:space-y-4">
        <h4 className="text-xs sm:text-sm font-bold text-indigo-900 flex items-center gap-2">
            <Zap className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Compliance & Footer
        </h4>

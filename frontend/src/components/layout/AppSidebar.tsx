...
export function AppSidebar({ onClose }: AppSidebarProps) {
  ...
  {collapsed ? (
    <img src={settings?.logo_url || logo} alt={settings?.brand_name || "NotifyNow"} className="w-16 h-16 rounded-full object-contain" />
  ) : (
    <img src={settings?.logo_url || logo} alt={settings?.brand_name || "NotifyNow"} className="w-32 h-16 object-contain" />
  )}
</div>

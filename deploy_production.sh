...
# 💳 CCAVENUE SECURE GATEWAY CONFIGURATION
log "   Configuring CCAvenue Secure Gateway..."
CCAV_MID="4439856"
CCAV_ACCESS="AVLY91ND22BP48YLPB"
CCAV_KEY="CAA5D0A531F741CA1E90039172BF460B"

# Ensure keys exist or update them
grep -q "^CCAVENUE_MERCHANT_ID=" "$BACKEND_DIR/$ENV_FILE" || echo "CCAVENUE_MERCHANT_ID=$CCAV_MID" >> "$BACKEND_DIR/$ENV_FILE"
perl -i -pe "s|^CCAVENUE_MERCHANT_ID=.*|CCAVENUE_MERCHANT_ID=$CCAV_MID|g" "$BACKEND_DIR/$ENV_FILE"

grep -q "^CCAVENUE_ACCESS_CODE=" "$BACKEND_DIR/$ENV_FILE" || echo "CCAVENUE_ACCESS_CODE=$CCAV_ACCESS" >> "$BACKEND_DIR/$ENV_FILE"
perl -i -pe "s|^CCAVENUE_ACCESS_CODE=.*|CCAVENUE_ACCESS_CODE=$CCAV_ACCESS|g" "$BACKEND_DIR/$ENV_FILE"

grep -q "^CCAVENUE_WORKING_KEY=" "$BACKEND_DIR/$ENV_FILE" || echo "CCAVENUE_WORKING_KEY=$CCAV_KEY" >> "$BACKEND_DIR/$ENV_FILE"
perl -i -pe "s|^CCAVENUE_WORKING_KEY=.*|CCAVENUE_WORKING_KEY=$CCAV_KEY|g" "$BACKEND_DIR/$ENV_FILE"

grep -q "^BACKEND_URL=" "$BACKEND_DIR/$ENV_FILE" || echo "BACKEND_URL=$APP_URL/api" >> "$BACKEND_DIR/$ENV_FILE"
perl -i -pe "s|^BACKEND_URL=.*|BACKEND_URL=$APP_URL/api|g" "$BACKEND_DIR/$ENV_FILE"

grep -q "^FRONTEND_URL=" "$BACKEND_DIR/$ENV_FILE" || echo "FRONTEND_URL=$APP_URL" >> "$BACKEND_DIR/$ENV_FILE"
perl -i -pe "s|^FRONTEND_URL=.*|FRONTEND_URL=$APP_URL|g" "$BACKEND_DIR/$ENV_FILE"
